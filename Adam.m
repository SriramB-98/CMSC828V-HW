...
fprintf(fopen('Adam_metrics.text','w'), 'max|err| = %d, L2 err = %d\n',max(max(abs(err))),norm(err(:)));
pause(2);
%
figure(1);clf;
...
title('Computed solution from Adam');
xlabel('x','Fontsize',fsz);
ylabel('y','Fontsize',fsz);
saveas(gcf,'Adam_solution.png');
...
colorbar;
set(gca,'Fontsize',fsz);
title('Error of Adam w.r.t exact solution');
xlabel('x','Fontsize',fsz);
ylabel('y','Fontsize',fsz);
saveas(gcf,'Adam_error.png');
...
saveas(gcf,'Adam_convergence.png');
end
